...
ALC_ 20210321.py : ALTERED FROM  ALC_20210318.py

Add a separate tagged region representing necrotic tissue.



ALC_20210318.py : ALTERED FROM   ALC_20210316.py

Stetch the block using homogeneous Dirichlet bc on the apical face and inhomogeneous Dirichlet on the basal face, rather than inhomogeneous Neumann. 
The force on the Dirichlet basal surface can then be calculated using the procedure given by Christoph 15-16/03/2021.

Make the active contractile region into one single tagged region.
The cushion region is removed.
...
def parser():
    ...
    group.add_argument('--maxstretch',
                        type=float, default=0.,
                        help='Maximum displacement applied to the basal surface.')

    group.add_argument('--initfile',
                        type=str, default='none',
                        help='Initialisation file.')

    group.add_argument('--if_write_ss',
                        choices=[1, 0],
                        default=1,
                        help='Choose whether to save final state.')
...
def run(args, job):
    ...
    if args.if_write_ss:
        #cmd += ['-write_statef', args.ID+'/ss']
        cmd += ['-num_tsav', 1]
        cmd += ['-tsav[0]', args.duration]
    ...
    if args.initfile != 'none':
        cmd += ['-start_statef', args.initfile]
    ...
    File1 = open('My_element_selection.regele', 'w')
    ...
    File1.close()
    dyn_reg = ['-numtagreg', 1]
    dyn_reg += ['-tagreg[0].type', 4]
    dyn_reg += ['-tagreg[0].elemfile', 'My_element_selection']
    dyn_reg += ['-tagreg[0].tag', necrosisTag[0]]
    cmd += dyn_reg

    # --- set up mechanics materials------------------------------------------
    wedge, necrosis = setupMechMat(args,wedgeTags, necrosisTag)

    # --- define stimuli -----------------------------------------------------
    stim = SetupStimuli(geom)
    cmd += stim
    ...
    #         '-mechanic_nbc[0].trace', 'traces/dbc_cube_AL_slow',
    ...
    # # INHOMOGENEOUS Dirichlet BC - base
    ...
    #         '-mechanic_dbc[1].ux', 0,
    #         '-mechanic_dbc[1].uy', args.maxstretch,
    #         '-mechanic_dbc[1].uz', 0,
    ...
    # HOMOGENEOUS Dirichlet BC - face, 
    cmd += mesh.block_boundary_condition(geom, 'mechanic_dbc', 1, 'y',  lower=False, bath=args.immersed)
    cmd += ['-mechanic_dbc[0].name', 'basal_dbc.surf']
    cmd += ['-mechanic_dbc[0].bctype',     0,  # <-- 1 for inhomogeneous, 0 for homogeneous
            '-mechanic_dbc[0].apply_ux',   1,
            '-mechanic_dbc[0].apply_uy',   1,
            '-mechanic_dbc[0].apply_uz',   1,
    ...
    # --- setup EP regions --------------------------------------------------
    imps, gregs, ekregs = setupEP(wedgeTags, necrosisTag,  args)
    ...
    #if args.visualize:
    cmd += ['-gridout_i',    3]
            #'-stress_value', 4]   ; print('COMMAND-LINE ARGUMENTS FED TO job.carp(cmd): '); print(cmd)       Removed to define -stress_value as 16 to obtain stress tensor cf Christoph's email 15/3/21.
...
def build_wedge(args):
    ...
    active_tissue = mesh.BoxRegion((cf_0,ab_0,tm_0), (cf_1,ab_3,tm_3), tag=1)
    geom.add_region(active_tissue)
    ...
    wedgeTags = [1] #[11, 12, 13, 21, 22, 23, 31, 32, 33]
...
def setupMechMat(args,wedgeTags, necrosisTag):  #,cushTags):
    ...
    necrosis = model.mechanics.GuccioneMaterial(wedgeTags, 'wedge',  kappa=1000., a=1.0 )   # ##### Should this read necrosisTag ???
...
def setupEP(wedge, necrosis, args):
   ...
   imps += [ '-imp_region[1].num_IDs', len(necrosis)] #+len(cushion) ]
   for ind, reg in enumerate(necrosis): # + cushion):
...
def setupActive (Stress, EP, active_imp, veldep):
    ...
    if statefile:
        if not os.path.isfile(f):
            ...
            opts += [ '-imp_region[{0}].im_sv_init'.format(active_imp[0]), f ]
            opts += [ '-imp_region[{0}].im_sv_init'.format(active_imp[1]), f ]
...
def setupStressParams(EP, Stress, active_imp):
    ...
    if EP != 'GPB':
         ...
         if Stress == 'LandStress' or Stress == 'LandStress_Cai' or Stress == 'LandStress_Cai_lambda':
             ...
             p += ['T_ref=40,Ca_50ref=0.54,TRPN_50=0.5,n_TRPN=2.7,k_TRPN=0.14,n_xb=3.38,k_xb=4.9e-3']     #for active_imp[0]
             p += ['T_ref=0,Ca_50ref=0.54,TRPN_50=0.5,n_TRPN=2.7,k_TRPN=0.14,n_xb=3.38,k_xb=4.9e-3']      #for active_imp[1]
         else:
             # TanhStress
