...
r"""    ALTERED FROM   ALC_20210609.py

Isometric block contraction with necrotic fraction.

The necrotic fraction is distributed normally around "parent" centres. The mean spacing between parents is given by parameter necr_s. The stddev of the distrubution around each parent is necr_r. 
The parents are uniformly distributed over the block volume. A randomisation seed can be set to enforce reproducibility.
"""
...
def parser():
    ...
    group.add_argument('--necr_s',
                        type=int, default=15000,
                        help='Necrosis model: mean distance (um) between *parents*')

    group.add_argument('--necr_r',
                        type=int, default=1000,
                        help='Necrosis model: std dev (um) of necrotic element distribution around each  *parent*')
    
    group.add_argument('--randseed',
                        type=int, default=10,
                        help='Random number seed for generating Poisson clusters.')
...
def run(args, job):
    ...
    #Generate the element file:
    print(job.ID+f'NecrElem_frac{FracNecrotic}_s{Necr_s}_r{Necr_r}.regele')
    ALC.MakePoissonClusters_1D('block_i_ref', FracNecrotic, Necr_s, Necr_r, OutputFile=f'NecrElem_frac{FracNecrotic}_s{Necr_s}_r{Necr_r}.regele', randseed=args.randseed)
    
    dyn_reg = ['-numtagreg', 1]
    dyn_reg += ['-tagreg[0].type', 4]  # 4: define elements in elemfile.
    dyn_reg += ['-tagreg[0].elemfile', f'NecrElem_frac{FracNecrotic}_s{Necr_s}_r{Necr_r}']
    ...
    cmd += setupActive ('LandHumanStress', args.EP, active_imp, args.vd, args.Tref)
    ...
    #if args.visualize:
    cmd += ['-gridout_i',    3,
            '-stress_value', 48]   # 16 needed to output 3x3 tensor ()
...
def setupMechMat(args,wedgeTags,necrosisTags):
    ...
    necrosis = model.mechanics.GuccioneMaterial(necrosisTags, 'wedge',  kappa=1000., a=1.0 )   # ##### Should this read necrosisTag ???
...
def setupActive (Stress, EP, active_imp, veldep, Tref):
    ...
    activeStressPars = setupStressParams(EP,Stress,active_imp, Tref)
...
def setupStressParams(EP, Stress, active_imp, Tref):
    ...
    p += [f'Tref=0.0']      #for active_imp[1]     NECROTIC TISSUE
