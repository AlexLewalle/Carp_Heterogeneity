...
r"""    ALTERED FROM   ALC_20210521.py
...
def run(args, job):
    ...
    if np.isclose(FracNecrotic, 0.):             # AL added this condition 29/6/21
        necrosisTags = [666]
        ...
        for i, x in enumerate(MyMask):
            if x:
                File1.write(str(i) + '\n')
        File1.close()
        if np.isclose(args.fracnecrotic, 0.0):
            dyn_reg = ['-numtagreg', 1]
    ...
    if not np.isclose(args.fracnecrotic, 0.0):
