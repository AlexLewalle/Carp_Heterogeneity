...
Modified from ALC_20210421.py.
The sv initialisation file is generated for each pCa value by exec_20210515.sh and saved in "states/TT2_Cai_LandStress_pCa${pCa1}.sv"
...
def run(args, job):
    ...
    # cmd += ['-mechanic_dbc[1].name', 'basal_dbc']
    ...
    cmd += setupActive (args.Stress, args.EP, active_imp, args.vd,  args.pCa)    #15/5/21: AL added pCa
...
def setupActive (Stress, EP, active_imp, veldep,pCa):
    ...
    f = "/home/al12/Carp/Heterogeneity/states/TT2_Cai_LandStress_pCa{:.2f}.sv".format(pCa)
    f = "/home/al12/Carp/Heterogeneity/states/TT2_Cai_LandStress_20210515.sv"
    print('Initialisation sv file:   ' + f)
    statefile = True
...
def setupStressParams(EP, Stress, active_imp):
    ...
    if EP != 'GPB':
         ...
         if Stress == 'LandStress' or Stress == 'LandStress_Cai' or Stress == 'LandStress_Cai_lambda':
             ...
             p = 'T_ref=40,Ca_50ref=0.54,TRPN_50=0.5,n_TRPN=2.7,k_TRPN=0.14,n_xb=3.38,k_xb=4.9e-3,beta_0=0.,beta_1=0.' # betas =0 18/5/21
         else:
             # TanhStress
...
def setupEMCoupling (emCoupling):
    ...
    if(emCoupling == 'weak'):
        coupling = ['-mech_use_actStress', 1, '-mech_lambda_upd', 2, '-mech_deform_elec', 0]         #mech_lambda_upd 1->2 18/05/21
