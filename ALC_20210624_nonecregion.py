...
This is the same as ALC_20210624.py, except that there is no necrotic region. All the tissue is 'healthy'. This is to check whether, when Trefnec=Trefref=30 in ALC_20210624.py, we get the same basal force.
...
def run(args, job):
    ...
    for i, x in enumerate(MyMask):
        if x:
            File1.write(str(i) + '\n')
    File1.close()
    # dyn_reg = ['-numtagreg', 1]    REMOVE NECROTIC REGION
    ...
    active_imp = 0 # [0,1]  # define active imp region
...
def setupActive (Stress, EP, active_imp, veldep, Tref, Trefnec=0):
    ...
    im_pars += [ '-imp_region[{0}].plugins'.format(active_imp), Stress]  #[0]), Stress ]
...
def setupStressParams(EP, Stress, active_imp, Tref, Trefnec=0):
    ...
    params  += [ '-imp_region[{0}].plug_param'.format(active_imp), p[0] ] #), p[0] ]
