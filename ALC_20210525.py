...
r"""    ALTERED FROM   ALC_20210421.py

Changes made to ALC_20210421.py with Marina S:
1) LandStress -> LandHumanStress
2) -diffusionOn 0
3) removed cushion region
    num_imp_regions 1
    num_gregions 1
    num_ekregions 1
4) removed state variable initialisation
5) removed gvec
6) -mech_lambda_upd   changed from 1 to 2
7) tangent-mode changed from 1 (numerical tangent) to 0 (symbolic tangent)
8) -load_stiffening 1

Removed subregions of the wedge ==> only one region with tag = 1.
...
def parser():
    ...
    group.add_argument('--Stress', default='LandHumanStress',
                        choices=['LandStress', 'LandHumanStress'],
...
def run(args, job):
    ...
    cmd += [ '-mechDT', mydt* (not args.mechanics_off) ,
                '-spacedt', mydt,          # time interval for saved data
                '-timedt', mydt,
                '-dt', mydt]
    ...
    cmd += setupActive ('LandHumanStress', args.EP, active_imp, args.vd)
...
def setupStressParams(EP, Stress, active_imp):
    ...
    p += ['Tref=40']     #for active_imp[0]
    p += ['Tref=39.999']     #for active_imp[1]
